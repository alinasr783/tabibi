  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const builtinSectionKeys = Array.isArray(appointmentSections?.order)
        ? appointmentSections.order.map(String).filter((k) => k && !k.startsWith("custom:"))
        : ["medical_state", "history"];

      const customSections = Array.isArray(medicalFieldsConfig?.appointment?.customSections)
        ? medicalFieldsConfig.appointment.customSections
        : [];
      ...
      builtinSectionKeys.forEach((k) => {
        const title = appointmentSections?.items?.[k]?.title;
      ...
      builtinSectionKeys.forEach((sectionKey) => {
        const list = Array.isArray(medicalFieldsConfig?.appointment?.sectionTemplates?.[sectionKey])
          ? medicalFieldsConfig.appointment.sectionTemplates[sectionKey]
          : [];
      ...
      const allTemplates = flattenCustomFieldTemplates({ config: medicalFieldsConfig, context: "appointment" })
        .filter((t) => t?.enabled !== false)
        .map((t) => ({ ...t, section_id: String(t?.section_id || "") }));

      const existingAppointmentFieldsRaw = Array.isArray(appointment?.custom_fields) ? appointment.custom_fields : [];
      const existingAppointmentFields = mergeTemplatesIntoCustomFields(existingAppointmentFieldsRaw, allTemplates).map((f) => {
        const rawSection = typeof f?.section_id === "string" ? f.section_id : "";
        const mapped = rawSection === "default" ? "medical_state" : rawSection;
      ...
      existingAppointmentFields.forEach((f) => {
        if (!f?.id) return;
      ...
      console.error('Error in Appointment Intelligence:', error);
            ...
            >
              {chatLog.length === 0 && (
                <div className="text-center text-slate-400 py-8 text-sm">
                  <p>ابدا اديني معلومات عن الحجز وانا هنظم البيانات</p>
                </div>
              )}
